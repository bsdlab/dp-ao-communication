...
class AlphaOmegaAPI {
    ...
    // Buffer state queue: Contains boolean values for each run (buffer was empty in that run)
    std::deque<bool> _bufferStateQueue;
    int _n = 10; // Times of which the buffer can be filled with zeros without throwing warning
...
public:
    AlphaOmegaAPI() {
        ...
        lsl::stream_info _functionCallOutletInfo = lsl::stream_info("AOFctCallStream", "String", 1, lsl::IRREGULAR_RATE, lsl::cf_string, "id0");
        lsl::stream_info _dataOutletInfo = lsl::stream_info("AODataStream", "EEG", _nChannels, _samplingRate, lsl::cf_float32, "id1");
    ...
    void run() {
        ...
        while (true){
            // Sleep(_refreshRate);
            // Only query after sleeping for _refreshRate milliseconds to no overload the CPU
            std::this_thread::sleep_for(std::chrono::milliseconds(_refreshRate));

            // Fill buffer with data from AO
            funcRet = GetAlignedData(buffer, _bufferSize, &actualData, _channelsArr, _nChannels, &TS_Begin);
...
private:
    void singleRun(int16* buffer, int actualData) {
        ...
        std::vector<std::vector<float>> slicedData = sliceData(buffer, actualData, _nChannels);

        //  Push each sample and simultaneously read and execute incomming commands on AO via tcp connection
    ...
    static std::vector<std::vector<float>> sliceData(int16* data, int actualData, int n_channels)
    {
        // Get number of samples per channel
        int nChannelData = actualData / n_channels;

        // Init vector of shape (nChannelData, nChannels)
        std::vector<std::vector<float>> resData(nChannelData, std::vector<float>(n_channels, 0));
