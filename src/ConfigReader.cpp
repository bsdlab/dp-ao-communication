...
void configReader::readParametersFromConfig() {
  ...
  _samplingRate =
      22000; // For now just manually set it // readIntegerFromConfig(
             // _configFilePath, "samplingRate");
...
//_____________________________________________________________________________
int configReader::readIntegerFromConfig(const std::string &cfgFilePath,
...
//_____________________________________________________________________________
std::string configReader::readStringFromConfig(const std::string &cfgFilePath,
                                               std::string parameterName) {
  std::ifstream in(cfgFilePath);
  std::string param;
  std::string value;
...
//_____________________________________________________________________________
void configReader::readArrayFromConfig(const std::string &cfgFilePath,
                                       std::string parameterName,
                                       std::string delimiter, int *arr) {
  std::ifstream in(cfgFilePath);
  std::string param;
  std::string value;
  while (!in.eof()) {
